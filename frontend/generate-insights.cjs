...
const pages = [
  {
    name: 'CursedStorefronts',
    title: 'Cursed Storefronts',
    p:'The Cursed Storefronts & Multi-Dimensional Attribution'
  },
  {
    name: 'ReviewManipulation',
    title: 'Review Manipulation Syndicate',
    p:'Uncovering clusters of fake/purchased reviews.'
  },
  {
    name: 'OpenWorldSafari',
    title: 'Open-World Data Safari',
    p:'Exploring unique, untethered observations hidden inside Yelp database.'
  }
];
pages.forEach(p => {
...
const  = () => {
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in">
      <Breadcrumb items={[{ label: 'Insights' }, { label: 'Data Enrichment' }, { label: '' }]} />
      <div className="mt-8 mb-12">
        <h1 className="text-4xl font-display font-bold text-gray-900"></h1>
        <p className="mt-4 text-xl text-gray-600 max-w-3xl"></p>
      </div>
      <div className="bg-white p-8 rounded-xl shadow-sm text-center text-gray-500 mt-12 border border-gray-100">
        Future comprehensive analysis mapping will appear here. Maps and charts correlate exactly as the Weather Mood implementations.
      </div>
    </div>
  );
};
export default ;;
  fs.writeFileSync('src/pages/insights/' + p.name + '.tsx', content);
});
